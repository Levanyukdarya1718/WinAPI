...
CONST INT g_i_CONTROL_START_X = g_i_START_X + (g_i_BUTTON_SIZE + g_i_INTERVAL) * 4;
CONST INT g_i_CONTROL_START_Y = g_i_BUTTON_START_Y;

CONST INT g_i_WINDOW_WIDTH = g_i_DISPLAY_WIDTH + g_i_START_X * 2 +16;
CONST INT g_i_WINWOW_HEIGHT = g_i_DISPLAY_HEIGHT + g_i_START_Y * 2 + (g_i_BUTTON_SIZE + g_i_INTERVAL) * 4+38;


CONST CHAR* g_OPERATIONS[] = { "+", "-", "*", "/" };

CONST COLORREF g_DISPLAY_BACKGROUND[] = { RGB(0,0,100), RGB(0,0,0) };
CONST COLORREF g_DISPLAY_FOREGROUND[] = { RGB(255,0,0), RGB(0,0,250) };
CONST COLORREF g_WINDOW_BACKGROUND[] =  { RGB(0,0,150), RGB(0,50,0) };
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	HBITMAP hBackground = (HBITMAP)LoadImage(hInstance, "Picture\\cosmo.bmp", IMAGE_BITMAP, 0, 0,LR_LOADFROMFILE);
	...
	(
		...
		g_i_WINDOW_WIDTH, g_i_WINWOW_HEIGHT, //size; width, height
		NULL,
		NULL,
		hInstance,
		NULL
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		//..-âûõîä û ðîäèòåëüñêèé êàòàëîã
			
		HFONT hFont = CreateFont(
			g_i_FONT_HEIGHT, g_i_FONT_WIDTH,
			0, //Escapement - Íàêëîí øðèôòà â äåñÿòêàõ ãðàäóñîâ
			0, //Orientation
			FW_BOLD, //wEIGHT - ÒÎËÙÈÍÀ
			FALSE, //Italic - ÊÓÐÑÎÂ
			FALSE, //Underline - ïîä÷åðêíóòûé
			FALSE, //Strikeout -ïåðå÷åðêíóòûé
			ANSI_CHARSET,
			OUT_TT_PRECIS,
		...
		(
			...
			WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON|BS_BITMAP,
			g_i_CONTROL_START_X,
			g_i_CONTROL_START_Y+g_i_BUTTON_SIZE+g_i_INTERVAL,
			g_i_BUTTON_SIZE, g_i_BUTTON_SIZE,
			hwnd,
		...
		(
			...
			WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON|BS_BITMAP,
			g_i_CONTROL_START_X,
			g_i_CONTROL_START_Y+(g_i_BUTTON_SIZE+g_i_INTERVAL)*2,
	...
	case WM_COMMAND:
	{
		...
		{
			//TODO: åÑËÈ ÏÅÐÂÛÉ ÑÈÌÂÎË . ÒÎ ÏÎÑËÅÄÓÞÙÈÉ ÂÂÎÄ ÍÅÏÐÀÂÈËÜÍÛÉ
	...
	case WM_KEYDOWN:
	{
		...
		{
			SendMessage(GetDlgItem(hwnd, wParam - 0x69 + IDC_BUTTON_0), ABM_SETSTATE, TRUE, 0);
		}
		switch (wParam)
		{
		case VK_ADD:
		case VK_OEM_PLUS:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_PLUS), ABM_SETSTATE, TRUE, 0);
			break;
		case VK_SUBTRACT:
		case VK_OEM_MINUS:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_MINUS), ABM_SETSTATE, TRUE, 0);
			break;
		case VK_MULTIPLY:
		...
		case VK_DIVIDE:
		case VK_OEM_2:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_SLASH), ABM_SETSTATE, TRUE, 0);
			break;
		case VK_DECIMAL:
		case VK_OEM_PERIOD:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_POINT), ABM_SETSTATE, TRUE, 0);
			break;
		case VK_ESCAPE:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_CLR), ABM_SETSTATE, TRUE, 0);
			break;
		case VK_BACK:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_BSP), ABM_SETSTATE, TRUE, 0);
			break;
		case VK_RETURN:
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_EQUAL), ABM_SETSTATE, TRUE, 0);
			break;
		}
	...
	case WM_KEYUP:
	{
		...
		{
			if (wParam == 0x38)
				SendMessage(GetDlgItem(hwnd, IDC_BUTTON_ASTER), ABM_SETSTATE, FALSE, 0);
		...
		case VK_OEM_PLUS:  
			...
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_PLUS), ABM_SETSTATE, FALSE, 0);
			break;
		case VK_SUBTRACT:
		case VK_OEM_MINUS: 
			SendMessage(hwnd, WM_COMMAND, LOWORD(IDC_BUTTON_MINUS), 0); 
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_MINUS), ABM_SETSTATE, FALSE, 0);
			break;
		case VK_MULTIPLY: 
		...
		case VK_OEM_2:		
			...
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_SLASH), ABM_SETSTATE, FALSE, 0);
			break;
		case VK_DECIMAL: 
		case VK_OEM_PERIOD: 
			SendMessage(hwnd, WM_COMMAND, LOWORD(IDC_BUTTON_POINT), 0); 
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_POINT), ABM_SETSTATE, FALSE, 0);
			break;
		case VK_ESCAPE:		
			SendMessage(hwnd, WM_COMMAND, LOWORD(IDC_BUTTON_CLR), 0); 
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_CLR), ABM_SETSTATE, FALSE, 0);
			break;
		case VK_BACK:		
			SendMessage(hwnd, WM_COMMAND, LOWORD(IDC_BUTTON_BSP), 0); 
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_BSP ), ABM_SETSTATE, FALSE, 0);
			break;
		case VK_RETURN:		
			SendMessage(hwnd, WM_COMMAND, LOWORD(IDC_BUTTON_EQUAL), 0); 
			SendMessage(GetDlgItem(hwnd, IDC_BUTTON_EQUAL), ABM_SETSTATE, FALSE, 0);
			break;
		}
	}
	break;
	case WM_CONTEXTMENU:
	{
		...
		if (skin_index >= IDR_SQUARE_BLUE && skin_index <= IDR_METAL_MISTRAL)
		{
			...
			SendMessage(hEditDisplay, WM_CTLCOLORDLG, (LPARAM)hdcDisplay, (LPARAM)hEditDisplay);
