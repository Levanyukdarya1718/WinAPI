...
BOOL CALLBACK DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		//GetModuleHandle(NULL); возврпащает hInstance нашей программы
...
BOOL CALLBACK DlgProcAddItem(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			//GetParent (hwnd) возвращает родительское токно (HWND) указанного окна для укакзанногог окна
			...
			{
				INT result = MessageBox(hwnd, "Такое вхождене уже есть в списке, хотите ввести другое значение", "Info", MB_YESNO | MB_ICONQUESTION);
